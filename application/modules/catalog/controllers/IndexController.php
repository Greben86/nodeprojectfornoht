...
//        $this->view->menu = $domDoc->saveHTML();
    ...
    public function parseQueryString($query)
    {
        ...
        foreach ($words as $word) {
            $query.='(RecordID:'.$word . '* OR name:'.$word.'* OR article:'.$word.'*)';
            if ($word != end($words)) {
                $query.=' AND ';
            }
        }
    ...
    public function searchAction() {
        ...
        if ($this->getRequest()->isGet()) {
            ...
            if (!empty($data['query'])) {                
                ...
                $analyzer->addFilter(new Zend_Search_Lucene_Analysis_TokenFilter_LowerCaseUtf8());
                ...
                $result = null;
                try {
                    $result = $index->find($this->parseQueryString($data['query']));
